...
export default function AppIntro({ navigation }) {
  ...
  const renderDone = () => {
    return (
      <TouchableOpacity onPress={() => { onDone() }} style={[styles.lottieopacity,{  height: screenHeight}]}>
        <View style={styles.renderDone}>
          <LottieView style={styles.vanView}
...
const styles = StyleSheet.create({
  vanView: { height: 100, width: DeviceInfo.getDeviceType() === 'Tablet' ? 350 : 120, position: "relative", left: 30 },
  renderDone: { height: 100, width: 200, flexDirection: "row", alignItems: "center", justifyContent: "flex-end", },
  mainWrapper: { height: "100%", position: "relative" },
  lottieView: { height: screenHeight, width: DeviceInfo.getDeviceType() === 'Tablet' ? screenWidth - 80 : screenWidth, },
  imageView: { height: ScreenHeight, width: screenWidth, position: 'absolute', top: 0, left: 0, resizeMode: "stretch" },
  image: {
    height: screenHeight,
    width: screenWidth,
    resizeMode: "contain"
  ...
  lottieopacity:{ position: "absolute", right: DeviceInfo.getDeviceType() === 'Tablet' ? 60 : -50,  top: 0, marginTop: -30 },
